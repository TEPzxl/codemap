...
```text
现在开始 Stage 13：Graph Layout and Interaction Polish。

主要修改 web/。优化 React Flow 自动布局、节点和边样式、高亮、fit view、minimap 和 reset layout。不要修改 analyzer/graph/server 核心逻辑，不要修改 Graph JSON Schema。完成后运行 make check、web build，并说明人工验收结果。
```
