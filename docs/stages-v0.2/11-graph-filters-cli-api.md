...
```text
现在开始 Stage 11：Graph Filters in CLI and API。

统一 CLI、API、Graph builder 的 filtering options。不要改 analyzer 核心逻辑，不要改 Graph JSON Schema。默认行为必须保持 v0.1。完成后运行 make check 和 CLI/API filter 验收命令。
```
