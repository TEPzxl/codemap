...
```text
现在开始 Stage 15：Performance, Cache, Rescan, and Metadata。

新增 /api/meta 和 /api/rescan，展示项目分析状态，并保证 rescan 不破坏旧 index。不要改 analyzer 的核心解析算法。完成后运行 make check、go test -race 相关包、web build 和 API 验收命令。
```
